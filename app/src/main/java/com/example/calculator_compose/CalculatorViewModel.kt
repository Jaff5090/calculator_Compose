...
class CalculatorViewModel: ViewModel() {
    ...
    public fun onAction(action: CalculatorActions) {
        when(action) {
            is CalculatorActions.Number -> enterNumber(action.number)
            is CalculatorActions.Delete -> delete()
            is CalculatorActions.Clear -> state = CalculateState()
    ...
    private fun calculate() {
        ...
        if(number1 != null && number2 != null) {
            val result = when(state.operation) {
                ...
                is CalculatorOperation.Mutiplie -> number1 * number2
                is CalculatorOperation.Divise -> number1 / number2
                null -> return
            }
            state = state.copy(
